"use client";

import { useId } from "react";

type Props = {
  isFull?: boolean;
  isEmpty?: boolean;
  roundedReviewAvg?: number;
};

export const Star = ({
  isFull = false,
  isEmpty = false,
  roundedReviewAvg = 0,
}: Props) => {
  const uniqueId = useId();

  return (
    <svg
      fill="#35C5F0"
      width="1em"
      height="1em"
      preserveAspectRatio="xMidYMid meet"
      viewBox="0 0 24 24"
    >
      <defs>
        <path
          id={`star-path-${uniqueId}`}
          d="M11.9996 19.7201L6.32294 22.1251C5.5626 22.4472 5.005 22.0311 5.0755 21.2188L5.60855 15.0767L1.5671 10.421C1.02579 9.79745 1.24924 9.13855 2.04358 8.95458L8.04973 7.56354L11.2287 2.28121C11.6545 1.57369 12.3502 1.5826 12.7706 2.28121L15.9496 7.56354L21.9557 8.95458C22.7602 9.1409 22.9667 9.8053 22.4322 10.421L18.3907 15.0767L18.9238 21.2188C18.9952 22.0414 18.4271 22.4432 17.6764 22.1251L11.9996 19.7201Z"
        ></path>
        <clipPath id={`star-clip-${uniqueId}`}>
          <rect
            x="0"
            y="0"
            width={isFull ? 24 : isEmpty ? 0 : (roundedReviewAvg % 1) * 24}
            height="24"
          ></rect>
        </clipPath>
      </defs>
      <use xlinkHref={`#star-path-${uniqueId}`} fill="#DBDBDB"></use>
      <use
        clipPath={`url(#star-clip-${uniqueId})`}
        xlinkHref={`#star-path-${uniqueId}`}
      ></use>
    </svg>
  );
};
